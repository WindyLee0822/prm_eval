...
from transformers import AutoModelForCausalLM, AutoTokenizer, AutoModel,AutoConfig, TrainingArguments, Trainer,PreTrainedModel, LlamaConfig, LlamaModel, MistralConfig,MistralModel, LlamaTokenizer
...
def compute_metrics(dataset_name, scored_results, key_list):
            ...
            if not args.baseline:
                for kind in key_list:
                    if not use_sc:
                        ...
                        # for comps in splitted_completions:
                        #     selected_completions += comps
                        # assert len(selected_completions) == len(scored_results), (len(selected_completions), len(scored_results))
                    ...
                    if not use_sc:
                        ...
                        #     print(kind, f'bon@{n}', acc)
                        metrics[sc_key][n] = round(acc * 100, 1)
                    else:
                        correct,sumv = 0,0
                        # print(f"{acc_list=}")
                        # print(f"{len(splitted_completions)=}")
                        for ii in range(len(splitted_completions)):
                            # correct_dict = {k:k==splitted_completions[ii][0]['reference'] for k in set(output_list[ii*n:(ii+1)*n])}
                            ...
                            # print(output_list[ii*n:(ii+1)*n].index(select_answer), acc_list)
                            is_correct = select_answer == splitted_completions[ii][0]['reference'] # we have preprocessed the outputs: if correct, output==gt
                            # is_correct = correct_dict[select_answer]
                            correct += is_correct
                            sumv+=1
                            # if ii % 100 == 0:
                            #     if accelerator.is_local_main_process:
                            #         print(f"{answer_dict=}, {select_answer=}, gt={splitted_completions[ii][0]['reference']}, is_correct={is_correct}")
                        # print(correct, sumv)
                        acc = correct/sumv
                        # if accelerator.is_local_main_process:
                        #     print('*********')
                        #     print(f'bon@{n}+SC', acc)
                        #     print('*********')
    ...
    for n in sample_nums:
        ...
        # if accelerator.is_local_main_process:
        #     print('*********')
        #     print(f'pass@{n}',pass_k,f'sc@{n}',sc)
        #     print('*********')
...
if __name__=='__main__':
    ...
    parser.add_argument("--type", type=str, default='dpo',choices=['dpo','prm-value','prm-llm','dpo-orm'])
    ...
    # parser.add_argument("--dpo-type", type=str, default='accumulative', choices=['single-step','accumulative'])
    ...
    parser.add_argument("--bon-dataset", type=str,default="math",choices=['math','gsm8k','qa'])
    ...
    for file_index, file_name in enumerate(file_list):
        ...
        if True or not args.baseline and not os.path.exists(output_file):
            if accelerator.is_local_main_process:
        ...
        if accelerator.is_main_process:
            os.makedirs("metrics-final", exist_ok=True)
            pd.DataFrame(results).to_json(f"metrics-final/{file_name.split('/')[-1][:-5]}-{save_name if 'orm' not in args.type else save_name+'-orm'}.json", orient="records", lines=True)
            f = open(f"metrics-final/{file_name.split('/')[-1][:-5]}-{save_name if 'orm' not in args.type else save_name+'-orm'}.txt", "w")
